...
import { IntegrationsNetlifyGeneratorSchema } from './schema';
...
export async function cloudflarePagesIntegrationGenerator(
  tree: Tree,
  options: IntegrationsNetlifyGeneratorSchema
) {
...
function getBuildSSRTargetNetlifyConfiguration(options: NormalizedOptions) {
  return {
    configFile: `${options.projectConfig.root}/adapters/netlify/vite.config.ts`,
  };
}
